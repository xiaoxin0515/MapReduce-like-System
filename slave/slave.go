...
type Maple_Task_Info struct {
	Host  string
    Msg   *Maple_Message
	Copy_cmd  string
}

type Juice_Task_Info struct {
	Host  string
	Msg   *Juice_Message
	Copy_cmd  string
}
...
type Slave struct {
	...
	Lock           sync.Mutex
	Maple_tasks    map[string]*Maple_Task_Info
	Juice_tasks    map[string]*Juice_Task_Info
}
...
func InitSlave(mas *master.SDFSMaster) *Slave {
	...
	var lock sync.Mutex
	task_map1 := make(map[string]*Maple_Task_Info)
	task_map2 := make(map[string]*Juice_Task_Info)
	...
	new_slave.Lock = lock
	new_slave.Maple_tasks = task_map1
	new_slave.Juice_tasks = task_map2
	return new_slave
}
...
func getlines(file_path string) int{
	counts := 0
...
func (self *Slave) Send_maple_tasks(maple_exe string, maple_file string, num_maples int, sdfs_prefix string) {
	task_map1 := make(map[string]*Maple_Task_Info)
	task_map2 := make(map[string]*Juice_Task_Info)
	self.Maple_tasks = task_map1
	self.Juice_tasks = task_map2
	logger.WriteLog("MapleJuice Command: Put Localfile to Master")
	log.Println("MapleJuice Command: Put Localfile to Master")
    num_lines := getlines(maple_file)
    if len(self.MemberList) < num_maples {
		log.Println("Error: Too Few Machines for Tasks")
		logger.WriteLog("Error: Too Few Machines for Tasks")
		return
	}
    task := self.MemberList[:num_maples]
    for tid := 0; tid < num_maples; tid++ {
    	msg := new(Maple_Message)
    	msg.T_id = tid
    	msg.CMD = "maple"
		...
		copy_cmd := fmt.Sprintf("tail -n +%s %s | head -n %s | sshpass -p XY19960218@uiuc ssh yaoxiao9@%s 'cat > /home/yaoxiao9/tmp/%s-%s'",
			strconv.Itoa(start), maple_file, strconv.Itoa(size), host, sdfs_prefix, maple_file)
		cmdWriter.Write([]byte(copy_cmd + "\n"))
		cmdWriter.Write([]byte("exit" + "\n"))
		cmd.Wait()
		task_now := &Maple_Task_Info{Host: task[tid].Address, Msg: msg, Copy_cmd: copy_cmd}
		self.Maple_tasks[host] = task_now
		self.start_Maple_task(task[tid], msg)
	}
}

func (self *Slave) start_Maple_task(mem *master.Member, msg *Maple_Message) {
	s,_ := json.Marshal(msg)
	send_message := string(s) + "<CMD>Maple"
	host := net.JoinHostPort(mem.Address, "8000")
	addr, err := net.ResolveUDPAddr("udp", host)
	logger.WriteLog("Sending Maple Task to" + host)
	if err != nil {
		log.Panic("Error when solving receiver IP", err)
	}
	con, err := net.DialUDP("udp", nil, addr)
	if err != nil {
		log.Panic("Error when dialing UDP address for Maple Task", err)
	}
...
func (self *Slave) start_Juice_task(mem *master.Member, msg *Juice_Message) {
	s,_ := json.Marshal(msg)
	send_message := string(s) + "<CMD>Juice"
	host := net.JoinHostPort(mem.Address, "8000")
	addr, err := net.ResolveUDPAddr("udp", host)
	logger.WriteLog("Sending Juice Task to" + host)
	if err != nil {
		log.Panic("Error when solving receiver IP", err)
	}
	con, err := net.DialUDP("udp", nil, addr)
	if err != nil {
		log.Panic("Error when dialing UDP address for Juice Task", err)
	}
	_, err = con.Write([]byte(send_message))
	if err != nil {
		log.Panic("Sending Join Request Through UDP Error:", err)
...
func getIntAbs(n uint32) uint32 {
	if n < 0 {
		return -n
	}
	return n
...
func (self *Slave) send_juice_tasks(juice_exe string, num_juices int, sdfs_dest_filename string, partition string, delete_input string) {
	task_map1 := make(map[string]*Maple_Task_Info)
	task_map2 := make(map[string]*Juice_Task_Info)
	self.Maple_tasks = task_map1
	self.Juice_tasks = task_map2

	if len(self.MemberList) < num_juices {
		log.Println("Too few machines for tasks")
		logger.WriteLog("Too few machines for tasks")
		return
	}

	tasks := self.MemberList[:num_juices]
	var gflist []string

	for key, _ := range self.SDFSMaster.File_matadata {
		gflist = append(gflist, key)
	}
	if partition == "hash" {
       hash_dict := make(map[uint32][]string)
       for _,file := range(gflist) {
		   hash_num := getIntAbs(hash(file)) & MaxInt32 % uint32(num_juices)
		   hash_dict[hash_num] = append(hash_dict[hash_num], file)
	   }
       for tid, _ := range(hash_dict) {
       	   msg := new(Juice_Message)
       	   msg.CMD = "juice"
       	   ...
       	   task := &Juice_Task_Info{Host: tasks[tid].Address, Msg: msg}
       	   self.Juice_tasks[tasks[tid].Address] = task
		   self.start_Juice_task(tasks[tid], msg)
	   }
	} else if partition == "range" {
		for tid := 0; tid < num_juices; tid++ {
			start := tid*len(gflist) / num_juices
			end := (tid+1)*len(gflist) / num_juices
			start += 1
			end += 1
			msg := new(Juice_Message)
			msg.CMD = "juice"
			...
			msg.Dest_file = sdfs_dest_filename
			msg.Delete = delete_input
			msg.Filelist = gflist[start:end+1]
			task := &Juice_Task_Info{Host: tasks[tid].Address, Msg: msg}
			self.Juice_tasks[tasks[tid].Address] = task
			self.start_Juice_task(tasks[tid], msg)
		}
	} else {
		log.Println("We cannnot recgonize partition you entered, going back")
		logger.WriteLog("We cannnot recgonize partition you entered, going back")
		return
	}
}
...
func (self *Slave) spawn_maple_task(msg *Maple_Message) {
	time.Sleep(2)
	fmt.Println("Running Map Task: Maple " + msg.Maple_exe)
	file_seg := fmt.Sprintf("/home/yaoxiao9/tmp/%s-%s",msg.Prefix, msg.Maple_file)
	fmt.Println(file_seg)
	fmt.Println(exists(file_seg))
    f_seg, err := os.OpenFile(file_seg, os.O_RDONLY, 0644)
	if err != nil {
		fmt.Println("What is wrong with everything")
		log.Fatal(err)
	}
	//defer to close when you're done with it, not because you think it's idiomatic!
	defer f_seg.Close()
	fmt.Println("What is wrong with everything 1")
    fmt.Println(f_seg)
	cmd := exec.Command(("./" + msg.Maple_exe))
	cmd.Dir = "/home/yaoxiao9/ourmp4/"+msg.Maple_exe
	fmt.Println(cmd.Dir)
	...
	err = cmd.Run();
	if err != nil {
		fmt.Println("The cmd running is wrong")
		log.Fatal(err)
	}
	map_out := strings.Split(string(buf.Bytes()), "\n")
	fmt.Println(map_out)
    for _, line := range(map_out) {
		if line == ""{continue}
    	key := strings.Split(line, " ")[0]
    	filename := "/home/yaoxiao9/tmp/" + msg.Prefix + "-" + key
	...
	cmdWriter2, _ := cmd2.StdinPipe()
	err = cmd2.Start()
	if err != nil {
		log.Fatal("error with writing to executing the bash command", err)
	}
	copy_cmd2 := "rm /home/yaoxiao9/tmp/" + msg.Prefix + "-" + msg.Maple_file
	cmdWriter2.Write([]byte(copy_cmd2 + "\n"))
	cmdWriter2.Write([]byte("exit" + "\n"))
	cmd2.Wait()
	print("Sleeping for maple job to run...")
	time.Sleep(10)
	print("Putting files")
	file_list := GetAllFile("/home/yaoxiao9/tmp")
	for _, file := range(file_list){
		self.put("/home/yaoxiao9/tmp"+ file, file)
	}
	time.Sleep(3)
	cmd3 := exec.Command("bash")
	cmdWriter3, _ := cmd3.StdinPipe()
	err = cmd3.Start()
	if err != nil {
		log.Fatal("error with writing to executing the bash command", err)
	}
	copy_cmd3 := "rm /home/yaoxiao9/tmp/*"
	cmdWriter3.Write([]byte(copy_cmd3 + "\n"))
	cmdWriter3.Write([]byte("exit" + "\n"))
	cmd3.Wait()
	fmt.Println("Maple Task Complete")
	logger.WriteLog("Maple Task Complete")
}

func (self *Slave) spawn_juice_task(msg *Juice_Message) {
	log.Println("Juice task start")
	for _, fn := range(msg.Filelist){
		self.Get(fn, "/home/yaoxiao9/tmp/"+fn)
	}
	f_w,err := os.OpenFile(msg.Dest_file, os.O_WRONLY|os.O_CREATE|os.O_APPEND, 0666)
	if err != nil {
		log.Fatal(err)
	}
	f := GetAllFile("/home/yaoxiao9/tmp")
	for _, fn := range(f){
		f_seg, err := os.OpenFile(("/home/yaoxiao9/tmp/" + fn), os.O_RDONLY, 0644)
		if err != nil {
			log.Fatal(err)
		}
		//defer to close when you're done with it, not because you think it's idiomatic!
		defer f_seg.Close()
		cmd := exec.Command(("./" + msg.Juice_exe))
		cmd.Dir = "/home/yaoxiao9/ourmp4/" + msg.Juice_exe
		buf := bytes.NewBuffer([]byte{})
		cmd.Stdout = buf
		cmd.Stdin = f_seg
		err = cmd.Run();
	...
	cmdWriter3.Write([]byte("exit" + "\n"))
	cmd3.Wait()
	del, _ := strconv.Atoi(msg.Delete)
	if del == 1{
       file_list := self.SDFSInfo.Ls_file()
       for _, item := range(file_list){
       	self.delete(item)
       	fmt.Println("Juice Task Complete: Delete")
	   }
	}else{fmt.Println("Juice Task Complete")}
}
		...
		for {
			...
			if len(receive) > 1 {
				...
				if command == "JOIN" {
					...
					var rec_msg *Maple_Message
					_ = json.Unmarshal([]byte(receive[0]), &rec_msg)
					fmt.Println("Start Spawning Maple Task")
					fmt.Println("received message is:")
					fmt.Println("maple exe:"+rec_msg.Maple_exe)
					fmt.Println("maple file:"+rec_msg.Maple_file)
					fmt.Println("sdfs:"+rec_msg.Prefix)
					self.spawn_maple_task(rec_msg)
				} else if command == "Juice" {
					var rec_msg *Juice_Message
					_ = json.Unmarshal([]byte(receive[0]), &rec_msg)
					fmt.Println("Start Spawning Juice task")
					self.spawn_juice_task(rec_msg)
				}
			} else {
...
func (self *Slave) CheckInput() {
	...
	for {
		...
		if input == "maple"{
			var maple_exe string
			var num_maples int
			var sdfs_prefix string
			var maple_file string
			_, err := fmt.Scanln(&maple_exe, &num_maples, &sdfs_prefix, &maple_file)
			if err != nil {
				fmt.Println("error with reading user's input")
				continue
			}
		...
		if input == "juice"{
			...
			var delete_input string
			var partition string
			_, err := fmt.Scanln(&juice_exe, &num_juices, &sdfs_dest_filename, &delete_input, &partition)
			if err != nil {
				fmt.Println("error with reading user's input")
				continue
			}
			if delete_input != "0" || delete_input != "1"{
				fmt.Println("delete_input must be 0 or 1")
				continue
			}
			self.send_juice_tasks(juice_exe, num_juices, sdfs_dest_filename, partition, delete_input)
		}
	}
...
func (self *Slave) Put_to_replica(replica_address string, local_filename string, sdfs_filename string,
	...
	cmdString := fmt.Sprintf("sshpass -p XY19960218@uiuc ssh yaoxiao9@%s 'cat >> %s' < %s",
		replica_address, "/home/yaoxiao9/"+SDFS_FILE_PREFIX+sdfs_filename, local_filename)
